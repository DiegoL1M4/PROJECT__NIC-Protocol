...
distancias = []
for node in rede:
    distancias.append( [node[0], math.sqrt((BS[0]-node[1])**2 + (BS[1]-node[2])**2), node[1], node[2]] )

tamSector = 20
contNodes = 0
corNum = 0
while(distancias != []):
    # Escolhe o nó mais longe
    maior = distancias[0]
    maiorDT = distancias[0][3]
    for node in distancias:
        if(maiorDT < node[3]):
            menor = node
            menorDT = node[3]

    # Calcula as distâncias do nó escolhido de todos os outros
    distanciasNode = []
    for node in rede:
        if(node[0] != maior[0]):
            distanciasNode.append( [node[0], math.sqrt((maior[2]-node[1])**2 + (maior[3]-node[2])**2)] )

    # Seleciona os nós mais próximos
    for k in range(len(distanciasNode)):
        # Escolhe o nó mais longe
        menor = distanciasNode[0]
        menorDT = distanciasNode[0][1]
        for node in distanciasNode:
            if(node[1] < menorDT):
                menor = node
                menorDT = node[1]
        ...
        distanciasNode.remove(menor)
        for node in distancias:
            if(node[0] == menor[0]):
                distancias.remove(node)
...
cores = ['green', 'black', 'red', 'gray','blue','green', 'black', 'red', 'gray','blue','green', 'black', 'red', 'gray','blue','green', 'black', 'red', 'gray','blue']
for node in rede:
